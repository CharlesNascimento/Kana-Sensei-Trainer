...
public class PreNetwork {
	...
	public static int[] normalizeStrokes(ArrayList<String> strokes) {
		...
		for (int i = 0; i < strokes.size(); i++) {
			...
			System.out.println("Direction: " + strokes.get(i) + " position: " + position + " i: " + i);
		}
	...
	/**
	 * Returns all the possible stroke patterns from the characters in the list,
	 * without repetition.
	 * 
	 * @param characters
	 * @return A map of stroke patterns and their normalized representations.
	 */
	public static HashMap<String, int[]> getStrokePatterns(ArrayList<Character> characters) {
		HashMap<String, int[]> patterns = new HashMap<>();

		for (Character character : characters) {
			int[] normalization = normalizeStrokes(character.getStrokes());

			patterns.put(character.getStrokes().toString(), normalization);
		}
