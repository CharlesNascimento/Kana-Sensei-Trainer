...
public abstract class Neuron {
    ...
    public void calculateOutput() {
        ...
        for (int i = 0; i < this.mInputTerminals.length; i++) {
            sum += (this.mInputTerminals[i] * this.mWeights[i]);
        }

        this.setOutput(this.applyActivationFunction(sum / (this.mLayer.getNeuronsCount())));
        /*if (this instanceof  OutputNeuron)
            System.out.println("calculateOutput " + getOutput());*/
    }

    /**
     * Applies the activation function to calculate the output of this neuron. In this case, the
     * used function is (1 - e**-2x) f(x) = (1 + e**-2x).
     *
     * @param x The sum of the multiplication between the input terminals and its respective weights.
     * @return The output of this neuron.
     */
    ...
    public void resetWeights() {
        ...
        for (int i = 0; i < this.mWeights.length; i++) {
            ...
            //Log.d("Neuron", "resetWeights " + this.mWeights[i]);
        }
    }
    ...
    public void adjustWeights(double learningRate) {
        ...
        for (int i = 0; i < this.mInputTerminals.length; i++) {
            ...
            //Log.d("Neuron", "adjustWeights " + newWeights[i]);
        }
